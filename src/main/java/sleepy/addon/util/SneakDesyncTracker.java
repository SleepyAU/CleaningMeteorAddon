...
public final class SneakDesyncTracker {
    ...
    private static void onPacketSend(PacketEvent.Send event) {
        if (!(event.packet instanceof ClientCommandC2SPacket packet)) return;
        switch (packet.getMode()) {
            case PRESS_SHIFT_KEY -> clientSentSneaking = true;
            case RELEASE_SHIFT_KEY -> clientSentSneaking = false;
            default -> { }
        }
    }
