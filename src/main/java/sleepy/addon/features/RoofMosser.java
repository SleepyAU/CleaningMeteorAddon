...
public final class RoofMosser extends Module {
    ...
    private long elytraEquipRequestedAtMs, manualTakeoffStartedAtMs;
    ...
    private void onPacketReceive(PacketEvent.Receive event) {
        if (!totemLog.get() || !(event.packet instanceof EntityStatusS2CPacket packet)
            || packet.getStatus() != 35 || mc.world == null || packet.getEntity(mc.world) != mc.player) return;

        mc.execute(() -> {
            if (isActive() && mc.getNetworkHandler() != null) {
                mc.getNetworkHandler().getConnection().disconnect(Text.literal("Roof Mosser: totem popped."));
            }
        });
    }
    ...
    private boolean useTakeoffRocket(long now) {
        if (!mc.player.isFallFlying() || mc.world == null || mc.interactionManager == null) return false;
    ...
    private boolean tickSneakDesyncGuard() {
        ...
        if (sneakRepairStage == SneakRepairStage.Idle) {
            ...
            sendSneakCommand(ClientCommandC2SPacket.Mode.PRESS_SHIFT_KEY);
        ...
        if (sneakRepairStage == SneakRepairStage.PressSent) {
            ...
            sendSneakCommand(ClientCommandC2SPacket.Mode.RELEASE_SHIFT_KEY);
    ...
    private void sendSneakCommand(ClientCommandC2SPacket.Mode mode) {
    ...
    private void configureSafetyModules() {
        ...
        // Meteor 0.5.8 has no totem-pop Auto Log setting. This version handles the local
        // entity-status packet directly in onPacketReceive without mutating Auto Log.
    }
    ...
    private void clearAutomationState() {
        ...
        elytraEquipRequestedAtMs = 0L; releaseManualTakeoffInput(); stopTakeoffSlowdown();
