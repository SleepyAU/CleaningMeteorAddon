...
public final class MossSpreader extends Module {
    ...
    private boolean prepareBoneMealCrafting() {
        ...
        int stagingHotbar = sourceSlot <= 8 ? sourceSlot : selectedSlot();
    ...
    private boolean isInventoryCleanseProtectedStack(ItemStack stack) {
        ...
        if (stack.isOf(Blocks.MOSS_BLOCK.asItem())) return true;
    ...
    private int findMossBlockSlot() {
        ...
        for (int i = 0; i < 36; i++) {
            ItemStack stack = mc.player.getInventory().getStack(i);
            if (stack.isOf(Blocks.MOSS_BLOCK.asItem())) {
                return i;
            }
        }
        return -1;
    ...
    private Block mossBlockForSlot(int slot) {
        ...
        if (block == Blocks.MOSS_BLOCK) return block;
        return null;
    }
